...
## 🕹️***K-means***
- ***Algorithm***🧠  
  给定一个类分配方案 $C$ ，确定每个类的均值向量： $\{g_1,g_2,\ldots,g_K\}$  
  给定 $K$ 个均值向量的集合 $\{g_1,g_2,\ldots,g_K\}$ ，把每个对象分配给距离均值最近的类  
  重复上述过程直到评价函数值不发生变化
   1. 初始化
      随机选择 $K$ 个种子，使得 $d(g_i,g_j)>d_{min}$     
   2. 类的分配  
      根据最近距离把点分配给各类  
      $$Cluster(u_i)=\underset{g_i\in\{g_1,g_2,\ldots,g_K\}}{argmin}(d(u_i,g_i))$$
   3. 更新中心  
      计算新的类中心
